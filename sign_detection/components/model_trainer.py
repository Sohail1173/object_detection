import os,sys
import yaml
from sign_detection.utils.main_utils import read_yaml_file
from sign_detection.logger import logging
from sign_detection.exception import signException
from sign_detection.entity.config_entity import ModelTrainerConfig
from sign_detection.entity.artifacts_entity import ModelTrainerArtifact
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self)->ModelTrainerArtifact:
        logging.info("Entred initiate_model_trainer method of ModelTrainer class")
        try:
            logging.info("unzippinng data")
            os.system("unzip data.zip")
            os.system("rm data.zip")

            with open("data.yaml","r") as stream:
                numm_classess=str(yaml.safe_load(stream)['nc'])
            model_config_file_name=self.model_trainer_config.weight_name.split(".")[0]
            print(model_config_file_name)

            config=read_yaml_file(f"yolov5/models/{model_config_file_name}.yaml")

            config["nc"]=int(numm_classess)
            ...
            logging.info("Exited initiate_model_trainer method of ModelTrainner class")
